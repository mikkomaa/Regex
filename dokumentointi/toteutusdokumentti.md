...
Yllä olevien aikavaativuusanalyysien perusteella tämä tavoite saavutettiin ohjelman, tarkemmin sanoen automaatin, toteutuksessa. Myös testausdokumentissa kuvatut suorituskykytestit vahvistavat asian. Käytännössä tämä tarkoittaa, että ohjelma toimii riittävän nopeasti megatavujenkin suuruisilla syötetiedostoilla.

Suurin osa ohjelman koodia liittyy säännöllisen lausekkeen käsittelyyn ennen automaatin ajamista. Käsittelyyn kuluva aika on kuitenkin käytännössä merkityksetön ohjelman normaalin käytön kannalta. Tältä osin ohjelmassa näkyykin olio-ohjelmoinnin suositeltujen käytäntöjen noudattaminen sen sijaan, että aikavaativuus olisi aggressiivisesti minimoitu. Aikavaativuus on kyllä alhaisimmassa mahdollisessa vaativuusluokassa O(n) suhteessa lausekkeen pituuteen, mutta lauseketta käsitellään useampi kerta kyseisen vaativuuden algoritmeilla. Olisi sinänsä täysin mahdollista kirjoittaa lausekkeen käsittely siten, että lauseke käytäisiin läpi vain kerran ennen automaatin luontia.
...
Automaatin ajamista ehkä nopeuttaisi hieman se, jos yksittäinen automaatin tila voisi kuvata yhden merkin sijasta myös merkkiväliä. Nyt esimerkiksi säännöllisessä lausekkeessa olevasta merkkivälistä [a-z] luodaan 26 merkkitilaa automaattiin. Jos samalla tilalla voisi kuvata merkkiväliä, tarvittaisiin tällöin vain yksi merkkitila. Nopeutuminen voisi näkyä ainakin suorituskykytesteissä lausekkeilla, joissa on useita laajoja merkkivälejä. Toisaalta muutos hieman monimutkaistaisi kaikkien tilojen käsittelyä automaatin ajon aikana, joten suorituskyvyn muutoksen suuruutta on vaikea arvioida tarkasti. Teoreettisesti tarkastellen muutos ei alentaisi säännöllisen lausekkeen pituuteen suhteutettua nykyistä aikavaativuutta O(n).
